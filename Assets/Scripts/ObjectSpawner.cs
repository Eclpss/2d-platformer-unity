...
public class ObjectSpawner : MonoBehaviour
{
    ...
    public GameObject[] objectPrefabs; //0=smalGem, 1=bigGem, 2=enemy and more
    public float bigGemProbibility = 0.2f; //20% chance spawning of big gem
    public float enemyProbibility = 0.1f; //10% chance spawning of enemy
    public int maxObjects = 5; //max amount of the spawning thingy
    public float gemLifeTime = 10f; //every 10 second our gems will fade away
    ...
    private void SpawnObject()
    {
        ...
        while (!validPositionFound && validSpawnPositions.Count > 0)
        {
            ...
            //this stop all our gems to clustering in one area
            ...
            if (validPositionFound)
            {
                ...
                // GameObject chosenPrefab = objectPrefabs[(int)objectType];
                ...
                {
                    StartCoroutine(DestroyObjectAfterTime(gameObject, gemLifeTime)); //destroy gems aftertime 
                }
            }
    ...
    //gather our valid spawn positions so where can we spawn our gems positions and every other position
    private void GatherValidPositions()
    {
        validSpawnPositions.Clear();
        BoundsInt boundsInt = tilemap.cellBounds; //basicly getting all the tile map size and shape
        ...
        for (int x = 0; x < boundsInt.size.x; x++)
        {
            ...
            {
                TileBase tile = allTiles[x + y * boundsInt.size.x]; //this grabs us the right tile
                if (tile != null)
                {
                    Vector3 place = start + new Vector3(x + 0.5f, y + 1.5f, 0);
                    validSpawnPositions.Add(place);
                    //so basicly we looping our x and y if we find a tile in the vector 3 place = start + new vector3(x+0.5f, y+2f, 0) that means we are in the right direction
                }
            }
        }
    }
